...
namespace MMVII
{
    namespace NS_Bench_RSNL
    {
        ...
        cParamMainNW::cParamMainNW() : mAmplGrid2Real(0.1),
                                       mAmplReal2Init(0.1),
                                       mNoiseOnDist(0.0),
                                       mFactXY(1, 1)
        {
        ...
        cMainNetwork<Type>::cMainNetwork(
            ...
            const std::vector<Type> &aWeightSetSchur) : mModeSSR(aMode),
                                                        mParamLSQ(aParamLSQ),
                                                        mBoxInd(aRect),
                                                        mX_SzM(mBoxInd.Sz().x()),
                                                        mY_SzM(mBoxInd.Sz().y()),
        ...
        template <class Type>
        void cMainNetwork<Type>::PostInit()
        {
        ...
        template <class Type>
        Type cMainNetwork<Type>::ObsDist(const tPNet &aPN1, const tPNet &aPN2) const
        {
            return Norm2(aPN1.TheorPt() - aPN2.TheorPt());
        }

        template <class Type>
        bool cMainNetwork<Type>::OwnLinkingFiltrage(const cPt2di &, const cPt2di &) const
        {
            return true;
        }

        template <class Type>
        bool cMainNetwork<Type>::AxeXIsHoriz() const
        {
        ...
        template <class Type>
        cPtxd<Type, 2> cMainNetwork<Type>::ComputeInd2Geom(const cPt2di &anInd) const
        {
        ...
        template <class Type>
        bool cMainNetwork<Type>::WithSchur() const { return mWithSchur; }
        template <class Type>
        int &cMainNetwork<Type>::Num() { return mNum; }

        template <class Type>
        const cSim2D<Type> &cMainNetwork<Type>::SimInd2G() const { return mSimInd2G; }
        template <class Type>
        const cParamMainNW &cMainNetwork<Type>::ParamNW() const { return mParamNW; }

        template <class Type>
        cResolSysNonLinear<Type> *cMainNetwork<Type>::Sys() { return mSys; }

        template <class Type>
        Type cMainNetwork<Type>::CalcResidual()
        {
            Type aSumResidual = 0;
        ...
        template <class Type>
        void cMainNetwork<Type>::AddGaugeConstraint(Type aWeightFix, bool CanMangle)
        {
        ...
        template <class Type>
        Type cMainNetwork<Type>::DoOneIterationCompensation(double aWeigthGauge, bool WithCalcReset, bool CanMangleCstr)

        {
            ...
            for (const auto &aPN1 : mVPts)
            {
                ...
                if (aPN1.mSchurPoint)
                {
                    ...
                    {
                        if (mWeightSetSchur.at(0) >= 0)
                            aSetIO.AddFixVarTmp(-1, aPTh1.x(), mWeightSetSchur.at(0)); // soft constraint-x  on theoreticall
                        if (mWeightSetSchur.at(1) >= 0)
                            aSetIO.AddFixVarTmp(-2, aPTh1.y(), mWeightSetSchur.at(1)); // soft constraint-y  on theoreticall
                        if (mWeightSetSchur.at(2) >= 0)
                            aSetIO.AddFixCurVarTmp(-1, mWeightSetSchur.at(2)); // soft constraint-x  on current
                        if (mWeightSetSchur.at(3) >= 0)
                            aSetIO.AddFixCurVarTmp(-2, mWeightSetSchur.at(3)); // soft constraint-y  on current
        ...
        template <class Type>
        const Type &cMainNetwork<Type>::CurSol(int aK) const
        {
        ...
        template <class Type>
        cPNetwork<Type>::cPNetwork(int aNumPt, const cPt2di &anInd, cMainNetwork<Type> &aNet) : mNumPt(aNumPt),
                                                                                                mInd(anInd),
                                                                                                ...
                                                                                                (aNet.AxeXIsHoriz() ? (mInd == cPt2di(0, 1)) : (aNet.WithSchur() ? 
                                                                                                (mInd == cPt2di(-1, 0)) : (mInd == cPt2di(1, 0))))),
        ...
        */
        ...
        template <class Type>
        void cPNetwork<Type>::MakePosInit(const double &aMulAmpl)
        {
        ...
        template <class Type>
        cPtxd<Type, 2> cPNetwork<Type>::PCur() const
        {
        ...
        template <class Type>
        const cPtxd<Type, 2> &cPNetwork<Type>::TheorPt() const { return mTheorPt; }
        template <class Type>
        const cPtxd<Type, 2> &cPNetwork<Type>::PosInit() const { return mPosInit; }

        template <class Type>
        bool cPNetwork<Type>::AreLinked(const cPNetwork<Type> &aP2) const
        {
