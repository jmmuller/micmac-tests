...
namespace MMVII
{
  ...
  public:
    ...
    static const std::vector<std::string> VNamesObs() { return {"PixelCoordinates", "IntensitePreIm", "IntensitePostIm"}; }
        ...
        const std::vector<tObs> &aVObs) // const
    {
      ...
      auto aValueIm = Apply_FormalBilinIm2D_Formula(aVObs, IndBilin, xIm, yIm);
      // take into account radiometric transform
      // auto aValueModele = aRadTr + aRadSc * vModelInit;
      auto aValueModele = aRadTr + vModelInit;
...
template <class TypeFunc,class TypeObs>   
         TypeFunc  Apply_FormalBilinIm2D_Formula
                 (
...
// std::vector<std::string> GetFormalBilinIm2D_NameObs(const std::string & aPrefix);
