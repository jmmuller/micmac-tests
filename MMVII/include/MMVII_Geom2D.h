...
namespace MMVII
{
    ...
    */
    ...
    template <class Type>
    inline cPtxd<Type, 2> operator*(const cPtxd<Type, 2> &aP1, const cPtxd<Type, 2> &aP2)
    {
        return cPtxd<Type, 2>(aP1.x() * aP2.x() - aP1.y() * aP2.y(), aP1.x() * aP2.y() + aP1.y() * aP2.x());
    }

    template <class Type>
    inline cPtxd<Type, 2> conj(const cPtxd<Type, 2> &aP1) { return cPtxd<Type, 2>(aP1.x(), -aP1.y()); }
    template <class Type>
    inline cPtxd<Type, 2> inv(const cPtxd<Type, 2> &aP1)
    {
        AssertNonNul(aP1);
        return conj(aP1) / Type(SqN2(aP1));
    }
    template <class Type>
    inline cPtxd<Type, 2> Rot90(const cPtxd<Type, 2> &aP) { return cPtxd<Type, 2>(-aP.y(), aP.x()); }
    template <class Type>
    inline cPtxd<Type, 2> operator/(const cPtxd<Type, 2> &aP1, const cPtxd<Type, 2> &aP2) { return aP1 * inv(aP2); }

    template <class T>
    T operator^(const cPtxd<T, 2> &aP1, const cPtxd<T, 2> &aP2)
    {
        return aP1.x() * aP2.y() - aP1.y() * aP2.x();
    }

    template <class T>
    cPtxd<T, 3> TP3z0(const cPtxd<T, 2> &aPt);
    template <class T>
    cPtxd<T, 2> Proj(const cPtxd<T, 3> &aPt);
    template <class T>
    cTriangle<T, 3> TP3z0(const cTriangle<T, 2> &aPt);
    template <class T>
    cTriangle<T, 2> Proj(const cTriangle<T, 3> &aPt);

    template <class T>
    inline cPtxd<T, 2> ToPolar(const cPtxd<T, 2> &aP1) ///<  From x,y to To rho,teta
    {
        AssertNonNul(aP1);
        return cPtxd<T, 2>(std::hypot(aP1.x(), aP1.y()), std::atan2(aP1.y(), aP1.x()));
    }
    template <class T>
    inline cPtxd<T, 2> ToPolar(const cPtxd<T, 2> &aP1, T aDefTeta) ///<  With Def value 4 teta
    {
        return IsNotNull(aP1) ? ToPolar(aP1) : cPtxd<T, 2>(0, aDefTeta);
    }
    template <class T>
    inline cPtxd<T, 2> FromPolar(const T &aRho, const T &aTeta)
    {
        return cPtxd<T, 2>(aRho * cos(aTeta), aRho * sin(aTeta));
    }
    template <class T>
    inline cPtxd<T, 2> FromPolar(const cPtxd<T, 2> &aP)
    {
        return FromPolar(aP.x(), aP.y());
    }

    template <class Type>
    inline cPtxd<Type, 2> PSymXY(const cPtxd<Type, 2> &aP)
    {
    ...
    template <class Type>
    cDenseMatrix<Type> MatOfMul(const cPtxd<Type, 2> &aP);

    template <class Type>
    class cSegment2DCompiled : public cSegmentCompiled<Type, 2>
    {
    public:
    ...
    */
    ...
    public:
        ...
        cHomot2D(const tPt &aTr, const Type &aSc) : mSc(aSc),
                                                    mTr(aTr)
        {
        }
    ...
    */
    ...
    public:
        ...
        cSim2D(const tPt &aTr, const tPt &aSc) : mTr(aTr),
                                                 mSc(aSc)
        {
        }
    ...
    public:
        ...
        cRot2D(const tPt &aTr, const Type &aTeta) : mTeta(aTeta),
    ...
    template <class Type, class tMap>
    cTriangle<Type, 2> ImageOfTri(const cTriangle<Type, 2> &, const tMap &);

    template <class Type>
    class cTriangle2DCompiled : public cTriangle<Type, 2>
    {
    public:
        typedef cPtxd<Type, 2> tPt;
    ...
    template <class Type>
    class cTriangulation2D : public cTriangulation<Type, 2>
    {
    public:
        typedef cPtxd<Type, 2> tPt;
    ...
    template <class Type>
    std::vector<cPtxd<Type, 2>> RandomPtsOnCircle(int aNbPts);
    /// Specialze for homogr, avoid singlularity in [-1,1]^2
    template <class Type>
    std::pair<std::vector<cPtxd<Type, 2>>, std::vector<cPtxd<Type, 2>>> RandomPtsHomgr(Type aAmpl = 1.5);
    ...
    template <class tMap>
    tMap RandomMapId(typename tMap::tTypeElem aEpsId);
