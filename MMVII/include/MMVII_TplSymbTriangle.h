...
namespace MMVII
{
    ...
    **/
    ...
    // std::vector<std::string> GetFormalBilinTriangle_NameObs(const std::string & aPrefix); // CHANGE HERE could have impact
    ...
    void Set_TriangleDeform_Obs(
        ...
        const cDataIm2D<TypeIm> &aDImPre, // image
        const cDataIm2D<TypeIm> &aDImPost // image
    )
    {
        ...
        SetOrPush(aVObs, aK0 + 2, (Type)aDImPre.GetV(aP0));
        SetOrPush(aVObs, aK0 + 3, (Type)aDImPost.GetV(aP0));
    }

    constexpr size_t FormalBilinTriangle_NbObs = 4;
};
